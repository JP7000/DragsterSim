...
function drawSpeedChart(canvas, times, speeds, dists) {
  ...
  ctx.fillStyle = ambrFill; ctx.fill();
  ctx.beginPath(); ctx.moveTo(tx(times[0]), ty(dists[0], maxD));
  for (let i=1; i<times.length; i++) ctx.lineTo(tx(times[i]), ty(dists[i], maxD));
  ctx.strokeStyle = ambrLine; ctx.lineWidth = 2; ctx.stroke();
  // Speed (teal)
  ctx.beginPath(); ctx.moveTo(tx(times[0]), ty(speeds[0], maxV));
  for (let i=1; i<times.length; i++) ctx.lineTo(tx(times[i]), ty(speeds[i], maxV));
  ctx.lineTo(tx(times[times.length-1]), PAD.t+cH); ctx.lineTo(tx(times[0]), PAD.t+cH); ctx.closePath();
  ctx.fillStyle = tealFill; ctx.fill();
  ctx.beginPath(); ctx.moveTo(tx(times[0]), ty(speeds[0], maxV));
  for (let i=1; i<times.length; i++) ctx.lineTo(tx(times[i]), ty(speeds[i], maxV));
  ctx.strokeStyle = tealLine; ctx.lineWidth = 2; ctx.stroke();
  // Axis labels
  ctx.font = '9px "Space Mono", monospace';
  ctx.textAlign = 'right'; ctx.textBaseline = 'middle'; ctx.fillStyle = tealLine;
  for (let i=0; i<=4; i++) ctx.fillText((maxV*(4-i)/4).toFixed(1), PAD.l-4, PAD.t+(i/4)*cH);
  ctx.textAlign = 'left'; ctx.fillStyle = ambrLine;
...
function drawThrustChart(F0, tau, dur) {
  ...
  const cW = W-PAD.l-PAD.r, cH = H-PAD.t-PAD.b;
  ctx.fillStyle = isLight ? '#f4f7fa' : '#0c0c0c'; ctx.fillRect(0,0,W,H);
  const pts = [];
  ...
  ctx.strokeStyle = isLight ? '#d0dbe6' : '#1a1a1a'; ctx.lineWidth=1;
  for (let i=0;i<=4;i++) {
    const y = PAD.t+(i/4)*cH;
    ctx.beginPath(); ctx.moveTo(PAD.l,y); ctx.lineTo(PAD.l+cW,y); ctx.stroke();
  }
  ...
  ctx.fillStyle = isLight ? 'rgba(204,0,34,0.08)' : 'rgba(232,0,45,0.1)'; ctx.fill();
  ctx.beginPath(); ctx.moveTo(tx(pts[0].t), ty(pts[0].f));
  for (let i=1;i<pts.length;i++) ctx.lineTo(tx(pts[i].t), ty(pts[i].f));
  ctx.strokeStyle = redLine; ctx.lineWidth=2; ctx.stroke();
  ctx.fillStyle = redLine; ctx.font='9px "Space Mono",monospace';
  ctx.textAlign='left'; ctx.textBaseline='bottom';
  ctx.fillText(F0.toFixed(1)+'N peak', tx(0)+4, ty(F0)-2);
  ctx.textAlign='right'; ctx.textBaseline='middle'; ctx.fillStyle = redLine;
  for (let i=0;i<=4;i++) ctx.fillText((maxF*(4-i)/4).toFixed(1), PAD.l-4, PAD.t+(i/4)*cH);
  ctx.textAlign='center'; ctx.textBaseline='top'; ctx.fillStyle = isLight ? '#6a7a8a' : '#444444';
...
function drawBarChart(canvas, labels, values, colours) {
  ...
  const ctx = canvas.getContext('2d'); ctx.scale(dpr, dpr);
  const cW = W - PAD.l - PAD.r, cH = H - PAD.t - PAD.b;
  ctx.fillStyle = isLight ? '#ffffff' : '#0c0c0c'; ctx.fillRect(0, 0, W, H);
  ...
  ctx.strokeStyle = isLight ? '#e0e8f0' : '#1a1a1a'; ctx.lineWidth = 1;
  for (let i=0; i<=3; i++) {
    const y = PAD.t + (i/3)*cH;
    ctx.beginPath(); ctx.moveTo(PAD.l, y); ctx.lineTo(PAD.l+cW, y); ctx.stroke();
    ctx.font = '9px "Space Mono",monospace'; ctx.textAlign = 'right'; ctx.textBaseline = 'middle';
  ...
  values.forEach((val, i) => {
    ...
    ctx.fillStyle = colours[i]+'44'; ctx.strokeStyle = colours[i]; ctx.lineWidth = 2;
...
function toggleWheelMode() {
  var isMoi = $('wheel-mode').value === 'moi';
  $('annular-opts').style.display = isMoi ? 'none' : 'block';
  $('moi-opts').style.display     = isMoi ? 'block' : 'none';
  ['wf-mass', 'wr-mass'].forEach(function(id) {
    var el = $(id); if (!el) return;
    el.disabled = isMoi;
    el.style.opacity = isMoi ? '.35' : '1';
  });
}
function setMoiUnit(unit) {
  _moiUnit = unit;
  var label = unit === 'gcm2' ? 'g·cm²' : 'kg·m²';
  $('moi-unit-lbl').textContent  = label;
  $('moi-unit-lbl2').textContent = label;
  var active   = 'rgba(232,0,45,0.18)';
  var inactive = 'var(--surface)';
  $('moi-btn-gcm2').style.background = unit === 'gcm2' ? active : inactive;
  $('moi-btn-gcm2').style.color      = unit === 'gcm2' ? 'var(--accent)' : 'var(--muted)';
  $('moi-btn-gcm2').style.fontWeight  = unit === 'gcm2' ? '700' : '400';
  $('moi-btn-kgm2').style.background = unit === 'kgm2' ? active : inactive;
...
function runSim() {
  ...
  // Show forces data, hide empty state SVG
  ...
  _prevFinishT = tMid;
  const inertiaNote = $('wheel-mode').value === 'moi' ? 'user-supplied wheel MOI' : 'annular-disc PLA wheel inertia';
  $('range-note').innerHTML =
    ...
    'Model v2: 8g CO\u2082 depletes proportionally to thrust \u00b7 dynamic m_eff \u00b7 ' + inertiaNote + ' \u00b7 \u03bcr rolling resistance. ' +
...
// ── Saved runs ─────────────────────────────────────────────────
const RUN_COLOURS = ['#00D2BE','#E8002D','#FF8000','#FFD700','#c084fc','#39B54A','#fb923c','#34d399'];
let savedRuns = [];
function saveRun() {
  ...
  const runData = Object.assign({}, window._lastRun, { label, colour, sT: window._lastRun.sT.slice(), sD: window._lastRun.sD.slice() });
...
function drawReplayFrame(simT) {
  ...
  const isLight=document.body.classList.contains('light');
  ctx.fillStyle=isLight?'#b0bec8':'#0c0c0c'; ctx.fillRect(0,0,cW,cH);
  _replayRuns.forEach((run,i)=>{
    const y=8+i*(LANE_H+LANE_GAP);
    ctx.shadowBlur=0;
    ctx.fillStyle=isLight?'#ffffff':'rgba(255,255,255,0.04)';
    rrect(ctx,PAD_L,y,trackW,LANE_H,3); ctx.fill();
    ctx.strokeStyle=isLight?'#888888':'rgba(255,255,255,0.07)'; ctx.lineWidth=1; ctx.setLineDash([]);
    rrect(ctx,PAD_L,y,trackW,LANE_H,3); ctx.stroke();
    ctx.strokeStyle=isLight?'rgba(0,0,0,0.3)':'rgba(255,255,255,0.15)'; ctx.lineWidth=1; ctx.setLineDash([3,3]);
    ...
    ctx.shadowColor=run.colour; ctx.shadowBlur=8; ctx.fillStyle=run.colour;
    rrect(ctx,carX-CAR_W,carY-CAR_H/2,CAR_W,CAR_H,2); ctx.fill(); ctx.shadowBlur=0;
    ctx.fillStyle=isLight?'#000000':'rgba(255,255,255,0.35)'; ctx.font='8px "Space Mono",monospace';
...
function updateReplayTimeDisplay(simT) {
  ...
  disp.innerHTML=_replayRuns.map(function(run){
    ...
    return '<span style="font-family:var(--font-mono);font-size:.7rem;color:' + col + ';">' + run.label.slice(0,12) + ': ' + run.displayT.toFixed(4) + 's</span>';
  }).join('');
}
...
function validateInputs() {
  ...
  if (isMoi) {
    ['moi-front','moi-rear'].forEach(function(id) {
      const el=$(id); if(!el) return;
      const val=parseFloat(el.value);
      const label = id==='moi-front' ? 'Front wheel MOI' : 'Rear wheel MOI';
      if(isNaN(val)||val<=0) { errors.push(label + ' must be a positive number.'); el.classList.add('invalid'); }
    });
  }
  ...
  if(errors.length>0) {
    banner.innerHTML='⚠ Please fix the following:<br>'+errors.map(function(e){ return '• '+e; }).join('<br>');
...
window.addEventListener('load', () => {
  try { applyMuPreset(); applyMuBorePreset(); applyMuBodyPreset(); updateAxleSetup(); toggleWheelMode(); } catch(e) { console.warn('Init error:', e); }
...
window.addEventListener('resize', () => {
  ...
  _resizeTimer = setTimeout(() => {
    ...
    if ($('thrust-card') && $('thrust-chart').style.display !== 'none') {
